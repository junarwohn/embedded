...
int do_mem_mtest (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	for (;;) {
		 ...
		 */
		for (pattern = 1, offset = 0, pattern_tmp = 1; offset < num_words; pattern++, offset++, pattern_tmp++) {
		    //WATCHDOG_RESET();
		    temp = start[offset];
		    if (temp != pattern) {
			printf ("\nFAILURE1 (read/write) @ 0x%.8lx:"
				" expected 0x%.8lx, actual 0x%.8lx, expected tmp 0x%.8lx, offset 0x%.8lx)\n",
				(ulong)&start[offset], pattern, temp, pattern_tmp, offset);
			//return 1;
		    }
